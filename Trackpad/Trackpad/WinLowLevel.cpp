...
void WinLowLevel::MakeMessage(const char* msg) {
	HWND hwnd = GetActiveWindow();
	MessageBox(hwnd, (LPCWCHAR)msg, L"ERROR", MB_OK);
}
...
void WinLowLevel::RClick(bool pressed) {
	INPUT input = { 0 };
	::ZeroMemory(&input, sizeof(INPUT));
	input.type = INPUT_MOUSE;
...
bool WinLowLevel::IsRealKeyboard()
{
	return realKeyboard;
}

bool WinLowLevel::isKeyPressed(sf::Keyboard::Key Key)
{
	return ::isKeyPressedState((int)Key);
}